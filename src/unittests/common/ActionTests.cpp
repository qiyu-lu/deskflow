...
void ActionTests::keyboardBroadcastText_selectedScreens()
{
  Action action;
  action.setType(static_cast<int>(Action::Type::keyboardBroadcast));
  action.setKeyboardBroadcastMode(static_cast<int>(Action::MouseBroadcastMode::on));
  action.setHaveScreens(true);
  action.addScreen(QStringLiteral("client1"));
  action.addScreen(QStringLiteral("client2"));

  QCOMPARE(action.text(), QStringLiteral("keyboardBroadcast(on,client1:client2)"));
}

void ActionTests::keyboardBroadcastSettings_roundTripsMode()
{
  QTemporaryDir temporaryDir;
  QVERIFY(temporaryDir.isValid());
  ...
  QSettings settings(temporaryDir.filePath(QStringLiteral("action.ini")), QSettings::IniFormat);
  Action saved;
  saved.setType(static_cast<int>(Action::Type::keyboardBroadcast));
  saved.setKeyboardBroadcastMode(static_cast<int>(Action::MouseBroadcastMode::off));
  saved.setRestartServer(false);
  saved.saveSettings(settings);
  settings.sync();
  ...
  Action loaded;
  loaded.loadSettings(settings);
  QCOMPARE(loaded.type(), static_cast<int>(Action::Type::keyboardBroadcast));
  QCOMPARE(loaded.keyboardBroadcastMode(), static_cast<int>(Action::MouseBroadcastMode::off));
}
